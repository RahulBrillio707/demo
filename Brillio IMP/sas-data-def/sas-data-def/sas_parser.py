...
class SasParser:
    ...
    def parse_unknown_statement(self) -> Optional[Dict[str, Any]]:
        """Parse unknown/unrecognized statement with support for inline macro constructs"""
        ...
        # Check if this might contain inline macro constructs
        if self._statement_contains_inline_macro():
            return self.parse_mixed_statement()
    ...
    def _statement_contains_inline_macro(self) -> bool:
        """Check if the current statement contains inline macro constructs"""
        # Look ahead to see if we find %DO, %END, or other macro tokens before semicolon
        lookahead_pos = 0
        max_lookahead = 50
        
        while lookahead_pos < max_lookahead:
            token = self.peek_token(lookahead_pos)
            if not token:
                break
            ...
            # Check for inline macro constructs
            if token.token_type in [TokenType.KWM_DO, TokenType.KWM_END, 
                                  TokenType.KWM_IF, TokenType.KWM_THEN, TokenType.KWM_ELSE]:
                print(f"[DEBUG] Found inline macro token at lookahead {lookahead_pos}")
                return True
    ...
    def parse_mixed_statement(self) -> Dict[str, Any]:
        """Parse statement that contains both SAS and inline macro constructs"""
        start_token = self.current_token()
        print(f"[DEBUG] parse_mixed_statement at pos {self.pos}")
        
        # Parse this as a complex mixed statement with inline macro constructs
        parts = []  # Will contain both text parts and macro constructs
        current_text = []
        ...
        while (not self.at_end() and 
            ...
            if self.match_token_type(TokenType.KWM_DO):
                ...
                # Parse the inline %DO construct
                inline_do = self.parse_inline_macro_do()
                if inline_do:
                    parts.append(inline_do)
                    
            elif self.match_token_type(TokenType.KWM_END):
                # This %END should be handled by the inline DO parser
                # If we encounter it here, it's unexpected
                print(f"[WARNING] Unexpected %END in mixed statement at pos {self.pos}")
                current_text.append(self.get_token_text(token))
                self.advance()
        ...
        if self.match_token_type(TokenType.SEMI):
            ...
            parts.append({
                'type': 'text',
                'content': self.get_token_text(end_token)
            })
            self.advance()
        
        mixed_node = self.create_base_node(start_token, end_token)
        mixed_node.update({
            'type': 'SasMixedStatement',
            'parts': parts,
            'text': ''.join(part['content'] if part['type'] == 'text' 
                          else str(part) for part in parts)
        })
        
        print(f"[DEBUG] parse_mixed_statement completed, parts: {len(parts)}")
        return mixed_node
    
    def parse_inline_macro_do(self) -> Dict[str, Any]:
        """Parse inline %DO construct (like %DO i=1 %TO n; ... %END;)"""
        start_token = self.current_token()
        print(f"[DEBUG] parse_inline_macro_do at pos {self.pos}")
        
        # Use the existing macro DO parser but mark it as inline
        do_block = self.parse_macro_do_statement()
        if do_block:
            do_block['inline'] = True  # Mark as inline construct
    ...
    def parse_macro_def(self) -> Dict[str, Any]:
        ...
        while (not self.at_end() and 
            ...
            if self.match_token_type(TokenType.KW_DATA):
                ...
                print(f"[DEBUG] Found %END token in macro body - this should end a DO block")
                # %END without matching %DO - treat as unknown statement for error recovery
                unknown_stmt = self.parse_unknown_statement()
                if unknown_stmt:
                    macro_body.append(unknown_stmt)
            else:
    ...
    def parse_macro_do_statement(self) -> Dict[str, Any]:
        """Parse %DO/%END block with support for %DO %UNTIL, %DO %TO, and %DO %WHILE"""
        start_token = self.current_token()
        self.advance()  # consume '%do'
        
        # Check for different types of %DO loops
        do_type = "simple"  # simple, until, while, to
        condition = ""
        loop_variable = ""
        start_value = ""
        end_value = ""
        ...
        # Parse %DO %UNTIL condition
        if self.match_token_type(TokenType.KWM_UNTIL):
            do_type = "until"
            ...
            paren_count = 0
            while (not self.at_end() and 
                   not (self.match_token_type(TokenType.SEMI) and paren_count == 0)):
                token = self.current_token()
            ...
            condition = ''.join(condition_tokens).strip()
            print(f"[DEBUG] Parsed %DO %UNTIL condition: '{condition}'")
            
        # Parse %DO %WHILE condition 
        elif self.match_token_type(TokenType.KWM_WHILE):
            do_type = "while"
            ...
            condition = ''.join(condition_tokens).strip()
            print(f"[DEBUG] Parsed %DO %WHILE condition: '{condition}'")
            
        # Parse %DO var = start %TO end [%BY increment]
        elif (self.match_token_type(TokenType.IDENTIFIER) or 
              self.match_token_type(TokenType.MACRO_STRING)):
            # This could be a %DO i=1 %TO n type loop
            lookahead_pos = 1
            found_to = False
            
            # Look ahead to see if this is a %TO loop
            while lookahead_pos <= 10:
                lookahead_token = self.peek_token(lookahead_pos)
                if not lookahead_token:
                    break
                if lookahead_token.token_type == TokenType.KWM_TO:
                    found_to = True
                    break
                elif lookahead_token.token_type == TokenType.SEMI:
                    break
                lookahead_pos += 1
            ...
            if found_to:
                ...
                if (self.match_token_type(TokenType.IDENTIFIER) or 
                    self.match_token_type(TokenType.MACRO_STRING)):
                    loop_variable = self.get_token_text(self.current_token())
                    self.advance()
                ...
                start_tokens = []
                while (not self.at_end() and 
                       not self.match_token_type(TokenType.KWM_TO)):
                    start_tokens.append(self.get_token_text(self.current_token()))
                    self.advance()
                start_value = ''.join(start_tokens).strip()
                
                # Consume %TO
                if self.match_token_type(TokenType.KWM_TO):
                    self.advance()
                
                # Parse end value  
                end_tokens = []
                while (not self.at_end() and 
                       not self.match_token_type(TokenType.KWM_BY) and
                       not self.match_token_type(TokenType.SEMI)):
                    end_tokens.append(self.get_token_text(self.current_token()))
                    self.advance()
                end_value = ''.join(end_tokens).strip()
                
                # Parse %BY increment if present
                if self.match_token_type(TokenType.KWM_BY):
                    self.advance()  # consume '%by'
                    
                    increment_tokens = []
                    while (not self.at_end() and 
                           not self.match_token_type(TokenType.SEMI)):
                        increment_tokens.append(self.get_token_text(self.current_token()))
                        self.advance()
                    increment = ''.join(increment_tokens).strip()
                
                print(f"[DEBUG] Parsed %DO {loop_variable}={start_value} %TO {end_value} %BY {increment}")
        ...
        if self.match_token_type(TokenType.KWM_END):
            end_token = self.current_token()
            self.advance()
            print(f"[DEBUG] Found and consumed %END token at pos {self.pos}")
        else:
            print(f"[WARNING] Expected %END but didn't find it at pos {self.pos}")
        
        # Consume semicolon after %END if present
        if self.match_token_type(TokenType.SEMI):
            self.advance()
        
        # Create appropriate node based on %DO type
        do_node = self.create_base_node(start_token, end_token)
        
        if do_type == "until":
            do_node.update({
                'type': 'SasMacroDoUntilBlock',
                'condition': condition,
                'statements': statements
            })
        elif do_type == "while":
            do_node.update({
                'type': 'SasMacroDoWhileBlock', 
                'condition': condition,
                'statements': statements
            })
        elif do_type == "to":
            do_node.update({
                'type': 'SasMacroDoToBlock',
    ...
    def parse_single_macro_statement(self) -> Dict[str, Any]:
        ...
        if self.match_token_type(TokenType.KWM_LET):
            ...
            print(f"[DEBUG] Found %END token - this should end a DO block, returning None")
            # %END token should be handled by the DO block parser, not consumed here
            return None
        elif self.match_token_type(TokenType.IDENTIFIER):
            current_token = self.current_token()
            token_text = self.get_token_text(current_token) if current_token else "None"
    ...
    def parse_if_statement(self) -> Dict[str, Any]:
        ...
        # Check if the condition contains inline macro constructs
        if self._condition_contains_inline_macro():
            print(f"[DEBUG] IF statement contains inline macro constructs at pos {self.pos}")
            return self.parse_mixed_if_statement(start_token)
        
        # Parse condition (collect until THEN or semicolon)
        condition_tokens = []
        safety_counter = 0
        max_condition_tokens = 500  # Safety limit
        
        while (not self.at_end() and 
               not self.match_token_type(TokenType.KW_THEN) and
               not self.match_token_type(TokenType.SEMI) and
               safety_counter < max_condition_tokens):
            condition_tokens.append(self.get_token_text(self.current_token()))
            self.advance()
            safety_counter += 1
        
        if safety_counter >= max_condition_tokens:
            print(f"[WARNING] IF statement condition parsing hit safety limit at pos {self.pos}")
    ...
    def _condition_contains_inline_macro(self) -> bool:
        """Check if the IF condition contains inline macro constructs"""
        # Look ahead to see if we find %DO, %END before THEN or semicolon
        lookahead_pos = 0
        max_lookahead = 100
        ...
        while lookahead_pos < max_lookahead:
            ...
            if token.token_type in [TokenType.KW_THEN, TokenType.SEMI]:
                break  # End of condition
                
            # Check for inline macro constructs
            if token.token_type in [TokenType.KWM_DO, TokenType.KWM_END]:
                print(f"[DEBUG] Found inline macro token {token.token_type} in IF condition at lookahead {lookahead_pos}")
                return True
    ...
    def parse_mixed_if_statement(self, start_token) -> Dict[str, Any]:
        """Parse IF statement that contains inline macro constructs"""
        print(f"[DEBUG] parse_mixed_if_statement at pos {self.pos}")
        
        # Parse this as a complex mixed IF statement with inline macro constructs
        condition_parts = []  # Will contain both text parts and macro constructs
        current_text = []
        ...
        while (not self.at_end() and 
            ...
            if self.match_token_type(TokenType.KWM_DO):
                ...
                inline_do = self.parse_inline_macro_do()
                if inline_do:
                    condition_parts.append(inline_do)
                    
            elif self.match_token_type(TokenType.KWM_END):
                # This %END should be handled by the inline DO parser
                # If we encounter it here, it's unexpected
                print(f"[WARNING] Unexpected %END in IF condition at pos {self.pos}")
                current_text.append(self.get_token_text(token))
                self.advance()
        ...
        mixed_if_node = self.create_base_node(start_token, end_token)
        mixed_if_node.update({
            'type': 'SasMixedIfStatement',
            'condition_parts': condition_parts,
            'condition_text': ''.join(part['content'] if part['type'] == 'text' 
                                    else f"[MACRO_CONSTRUCT:{part.get('type', 'unknown')}]" 
                                    for part in condition_parts),
            'then_block': then_block,
            'else_block': else_block
        })
        
        print(f"[DEBUG] parse_mixed_if_statement completed, parts: {len(condition_parts)}")
        return mixed_if_node
...
def pretty_print_sas_ast(ast_node, indent=0, show_details=True):
    """
    Pretty print SAS AST in a readable table format
    """
    ...
    node_id = ast_node.get('id', '')[:8] + '...' if ast_node.get('id') else 'No ID'
    level = ast_node.get('level', 0)
    
    # Location info
    location = ast_node.get('location', {})
    line_start = location.get('line_start', '?')
    line_end = location.get('line_end', '?')
    col_start = location.get('col_start', '?')
    col_end = location.get('col_end', '?')
    
    # Get name/identifier based on type
    name = get_node_name(ast_node)
    
    # Get sub-constructs count
    sub_constructs = get_sub_constructs_info(ast_node)
    
    # Comments count
    comments_count = len(ast_node.get('comments', []))
    ...
    print(f"{prefix}├─ Lines: {line_start}-{line_end} (Cols: {col_start}-{col_end})")
    print(f"{prefix}├─ Comments: {comments_count}")
    print(f"{prefix}├─ Sub-constructs: {sub_constructs['summary']}")
    
    if show_details and sub_constructs['details']:
        print(f"{prefix}├─ Details: {sub_constructs['details']}")
...
def get_node_name(node):
    """Extract the main identifier/name from a node"""
    node_type = node.get('type', '')
    
    if node_type == 'SasModule':
        return node.get('encoding', 'Default') or 'Default'
    elif node_type == 'SasDataStep':
        return node.get('name', 'Unnamed')
    elif node_type == 'SasProcStep':
        return node.get('procedure_name', 'Unknown')
    elif node_type == 'SasMacroDef':
        return node.get('macro_name', 'Unnamed')
    elif node_type == 'SasLetStatement':
        name = node.get('name', '')
        value = node.get('value', '')
        return f"{name} = {value}"
    elif node_type == 'SasIncludeStatement':
        return node.get('path', 'Unknown path')
    elif node_type == 'SasSetStatement':
        datasets = node.get('datasets', [])
        if datasets:
            names = [d.get('name', 'Unknown') for d in datasets]
            return ', '.join(names)
        return 'No datasets'
    elif node_type == 'SasUnknownStatement':
        text = node.get('text', '')
...
def get_sub_constructs_info(node):
    """Get information about sub-constructs in this node"""
    result = {'summary': 'None', 'details': ''}
    
    # Check different containers
    containers = ['body', 'statements', 'macro_body']
    
    total_count = 0
    type_counts = {}
    
    for container in containers:
        items = node.get(container, [])
        if items:
    ...
    if total_count > 0:
        result['summary'] = f"{total_count} items"
        # Create details string
        details = []
        for item_type, count in type_counts.items():
            short_type = item_type.replace('Sas', '').replace('Statement', 'Stmt')
            details.append(f"{count}x{short_type}")
        result['details'] = ', '.join(details)
    
    return result


def print_sas_ast_tree(ast_node, indent=0, max_depth=10):
    """
    Print the AST as a hierarchical tree
    """
    if indent > max_depth:
        return
    
    # Print current node
    pretty_print_sas_ast(ast_node, indent, show_details=(indent < 3))
    
    # Recursively print children
    containers = ['body', 'statements', 'macro_body']
    
    for container in containers:
        items = ast_node.get(container, [])
        if items:
            prefix = "  " * indent
            print(f"{prefix}📁 {container.upper()}:")
            for item in items:
                if isinstance(item, dict):
                    print_sas_ast_tree(item, indent + 1, max_depth)
            print()  # Empty line after container


def print_sas_summary_table(ast_node):
    """
    Print a compact summary table of all constructs
    """
    constructs = []
    
    def collect_constructs(node, level=0):
        ...
        node_info = {
            ...
            'line_end': node.get('location', {}).get('line_end', '?'),
            'comments': len(node.get('comments', [])),
            'sub_items': sum(len(node.get(container, [])) for container in ['body', 'statements', 'macro_body'])
        }
        constructs.append(node_info)
        
        # Recursively collect from children
        for container in ['body', 'statements', 'macro_body']:
            for item in node.get(container, []):
                if isinstance(item, dict):
                    collect_constructs(item, level + 1)
    
    collect_constructs(ast_node)
    
    # Print table header
    print("="*100)
    print("📊 SAS AST SUMMARY TABLE")
    print("="*100)
    print(f"{'Type':<20} {'Name':<25} {'Lvl':<3} {'Lines':<10} {'Cmt':<3} {'Sub':<3} {'Preview':<30}")
    print("-"*100)
    
    # Print each construct
    for construct in constructs:
        type_short = construct['type'].replace('Sas', '').replace('Statement', 'Stmt')
        name = construct['name'][:24] if len(construct['name']) > 24 else construct['name']
        lines = f"{construct['line_start']}-{construct['line_end']}"
        preview = name[:29] if len(name) <= 29 else name[:26] + "..."
        
        print(f"{type_short:<20} {name:<25} {construct['level']:<3} {lines:<10} "
              f"{construct['comments']:<3} {construct['sub_items']:<3} {preview:<30}")
    
    print("-"*100)
    print(f"Total constructs: {len(constructs)}")
    print("="*100)
...
def build_graph_from_ast(ast, net, parent_id=None, level=0):
    if isinstance(ast, dict) and 'type' in ast:
        node_id = ast.get('id') or f"auto_{uuid.uuid4().hex[:8]}"
        node_type = ast.get('type', 'Unknown')
        name = ast.get('macro_name') or ast.get('name') or ast.get('procedure_name') or ast.get('variable') or ''
        loc = ast.get('location', {})
        start_line = loc.get('line_start', '')
        end_line = loc.get('line_end', '')
        label = f"{node_type}\n{name}" if name else node_type
        label += f"\nL{start_line}-{end_line}" if start_line and end_line else ""
        title = f"<b>{node_type}</b><br>{name}<br>L{start_line}-{end_line}<br><i>{loc.get('file', '')}</i>"

        # Add node
        net.add_node(node_id, label=label, title=title, level=level, color=type_color(node_type))
        ...
        for key, val in ast.items():
            if key in {'type', 'id', 'location', 'level', 'comments'}:
                continue
            if isinstance(val, list):
                for item in val:
                    if isinstance(item, dict) and 'type' in item:
                        build_graph_from_ast(item, net, node_id, level + 1)
            elif isinstance(val, dict) and 'type' in val:
                build_graph_from_ast(val, net, node_id, level + 1)


def type_color(node_type):
    return {
        'SasMacroDef': '#FF6B6B',
        'SasProcStep': '#4ECDC4',
        'SasDataStep': '#45B7D1',
        'SasLetStatement': '#96CEB4',
        'SasMacroCall': '#FECA57',
        'SasIfStatement': '#FF9FF3',
        'SasDoStatement': '#54A0FF',
        'SasSqlStatement': '#5F27CD',
        'SasUnknownStatement': '#DDA0DD',
        'SasAssignmentStatement': '#F368E0',
        'SasMacroIfStatement': '#FF9F43',
        'ROOT': '#2C2C54'
    }.get(node_type, '#CCCCCC')

def create_interactive_html(ast, output_path="output.html"):
    ...
    """)
    # Add root
    net.add_node("ROOT", label="🌳 SAS ROOT", level=0, color=type_color("ROOT"))
    build_graph_from_ast(ast, net, parent_id="ROOT", level=1)

    
    # Save to file
    net.write_html(output_path)
    print(f"✅ Saved interactive AST graph to {output_path}")

def extract_top_level_blocks(ast_json):
    """Extract top-level constructs with generic structure and full original AST in body"""

    if not isinstance(ast_json, dict) or ast_json.get('type') != 'SasModule':
        return []

    blocks = []
    body = ast_json.get('body', [])

    for i, construct in enumerate(body):
        if not isinstance(construct, dict):
            continue

        # Extract location info
        location = construct.get('location', {})
        start_line = location.get('line_start', 'unknown')
        end_line = location.get('line_end', 'unknown')

        # Extract name from various possible fields
        name = (construct.get('macro_name') or
                construct.get('name') or
                construct.get('procedure_name') or
                construct.get('libref') or
                construct.get('variable') or
                construct.get('path') or
                'unnamed')

        # Find what this block contains (for summary)
        contains = []
        container_fields = ['body', 'macro_body', 'statements', 'then_body', 'else_body', 'do_body']

        for field in container_fields:
            if field in construct:
                field_content = construct[field]
                if isinstance(field_content, list):
                    for item in field_content:
                        if isinstance(item, dict) and 'type' in item:
                            contains.append(item['type'])
                elif isinstance(field_content, dict) and 'type' in field_content:
                    contains.append(field_content['type'])

        # Create generic block structure with FULL original AST in body
        block = {
            'block_id': i + 1,
            'type': construct.get('type', 'Unknown'),
            'name': name,
            'start_line': start_line,
            'end_line': end_line,
            'contains': contains,
            'body': construct  # FULL original AST node
        }
...
#     test_code = '''/*This is the macro_1 sas program*/
# /* */
# /*Ben Corcoran 2019*/
# data test1;
#     set a;
...
#     result = parse_sas_code(test_code, "test.sas")
    
#     # Pretty print in different formats
#     print("🌳 HIERARCHICAL TREE VIEW:")
#     print_sas_ast_tree(result, max_depth=3)
    
#     print("\n" + "="*50)
#     print_sas_summary_table(result)